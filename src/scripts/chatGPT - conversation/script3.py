...
def extract_field_from_json(file_path, field, nested_key=None):
    """
    Extracts all instances of a specific field from a JSON file.
    If nested_key is provided, extracts the field only from objects within that key.
    """
    extracted_values = []
    try:
...
def main(base_folder, output_content_file, output_href_file):
    ...
    for file_path in find_specific_files(base_folder, ["message_1.json", "liked_posts.json"]):
        if file_path.endswith("message_1.json"):
            ...
            href_list.extend(extract_field_from_json(file_path, "href"))
...
output_content_txt = 'C:/Users/Miger Shkrepa/Desktop/python/runScript/collected_contents3.txt'
output_href_txt = 'C:/Users/Miger Shkrepa/Desktop/python/runScript/collected_hrefs3.txt'
