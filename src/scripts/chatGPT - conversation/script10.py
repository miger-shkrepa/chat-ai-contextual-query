        ...
        with open(file_path, 'r', encoding='utf-8') as file:
            ...
            if message_class:
                for outer_div in soup.find_all('div', class_=message_class):
                    nested_divs = outer_div.find_all('div', recursive=False)
                    if len(nested_divs) >= 2:  # Ensure we have at least two nested divs
                        content_div = nested_divs[1]
                        # Extract <div>content</div> directly
                        for inner_div in content_div.find_all('div', recursive=False):
                            content_text = inner_div.get_text(strip=True)
                            if content_text:
...
output_message_json = os.path.join(output_folder, "message_content10.json")
output_liked_json = os.path.join(output_folder, "liked_content10.json")
