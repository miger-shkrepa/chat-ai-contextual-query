            ...
            if "liked_posts" in data:
              for post in data["liked_posts"]:
                try:
                  ...
                  print(f"Warning: 'href' key not found in a post in {file_path}")
        except (json.JSONDecodeError, FileNotFoundError) as e:
          print(f"Error processing {file_path}: {e}")
...
if __name__ == "__main__":
  ...
  with open("../../../collectedData/Gemini - conversation/extracted_messages5.txt", "w", encoding='utf-8') as f:
    for message in all_messages:
      f.write(message + "\n")

  # Extract hrefs
  all_hrefs = extract_hrefs(main_folder_path)
  with open("../../../collectedData/Gemini - conversation/extracted_hrefs5.txt", "w", encoding='utf-8') as f:
