...
def extract_messages(main_folder):
  ...
  with open("../../../collectedData/Gemini - conversation/message_content7.json", "w", encoding='utf-8') as f:
              ...
              for post in data["likes_media_likes"]:
                if "string_list_data" in post:
                  for item in post["string_list_data"]:
                    try:
                      href = item["href"]
                      all_hrefs.append({"liked_content": href})
                    except KeyError:
                      print(f"Warning: 'href' key not found in an item in {file_path}")
        except (json.JSONDecodeError, FileNotFoundError) as e:
          print(f"Error processing {file_path}: {e}")

  with open("../../../collectedData/Gemini - conversation/liked_content7.json", "w", encoding='utf-8') as f:
    json.dump(all_hrefs, f, indent=2)
