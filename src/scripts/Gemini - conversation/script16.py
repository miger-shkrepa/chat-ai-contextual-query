    ...
    with open(file_path, 'r', encoding='utf-8') as f:
      ...
      for message_div in soup.find_all('div', class_="_3-95 _a6-p"):
        try:
          ...
          if len(nested_divs) >= 2:  # Ensure there are at least 2 nested divs
            content_div = nested_divs[2]
            content = content_div.find('div').text.strip()  # Extract text within the second div
            messages.append({"content": content})
...
def extract_messages(main_folder):
  ...
  with open("../../../collectedData/Gemini - conversation/message_content16.json", "w", encoding='utf-8') as f:
...
def extract_hrefs(main_folder):
  ...
  with open("../../../collectedData/Gemini - conversation/liked_content16.json", "w", encoding='utf-8') as f:
