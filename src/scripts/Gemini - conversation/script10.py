...
def extract_messages_from_html(file_path):
  ...
  # This is a simplified example and may need adjustments based on the HTML structure
  messages = []
  try:
    with open(file_path, 'r', encoding='utf-8') as f:
      soup = BeautifulSoup(f, 'html.parser')
      for message_tag in soup.find_all('div', class_='message'):  # Replace with actual class or tag
        try:
          content = message_tag.find('span', class_='content').text
          messages.append({"content": content})
        except AttributeError:
          print(f"Warning: Could not find 'content' in a message in {file_path}")
...
def extract_messages(main_folder):
  ...
  with open("../../../collectedData/Gemini - conversation/message_content10.json", "w", encoding='utf-8') as f:
...
def extract_hrefs(main_folder):
  ...
  with open("../../../collectedData/Gemini - conversation/liked_content10.json", "w", encoding='utf-8') as f:
